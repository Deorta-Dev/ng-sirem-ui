 ...
 * - `fixed` lo fija al scroll; `blur` le da fondo translúcido.
  ...
  template: `
          ...
          @for (item of visibleItems(); track item.label) {
            <div class="sirem-sidebar__hitem">
              @if (item.children?.length) {
                ...
                >
                  @if (item.svg) {
                    <span class="sirem-sidebar__svgicon" [innerHTML]="safeSvg(item.svg)" aria-hidden="true"></span>
                  } @else if (item.icon) {
                ...
                <ng-container *ngTemplateOutlet="linkTpl; context: { $implicit: item }"></ng-container>
              }
            </div>
          }
        </nav>
    ...
    >
      <div class="sirem-sidebar__brand">
        <span
          class="sirem-sidebar__logo"
          aria-hidden="true"
        >
          {{ brandInitial() }}
        </span>
        <span class="sirem-sidebar__label sirem-sidebar__brand-name">{{ brand() }}</span>
        <button
          type="button"
          (click)="collapsed.set(!collapsed())"
          class="sirem-sidebar__collapse"
